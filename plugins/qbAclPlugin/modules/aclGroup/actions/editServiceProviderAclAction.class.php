...
class AclGroupEditServiceProviderAclAction extends AclGroupEditDefaultAclAction
{
    ...
    foreach ($this->request->acl as $key => $value)
    {
      ...
      if (1 == preg_match('/([\w]+)_(.*)/', $key, $matches))
      {
        ...
        if (QubitAcl::INHERIT != $value && isset(QubitAcl::$ACTIONS[$action]))
        {
          ...
          $aclPermission->objectId = QubitServiceProvider::ROOT_ID;
  ...
  public function execute($request)
  {
    ...
    // Always include root service provider permissions
    $this->serviceProviders = array(QubitServiceProvider::ROOT_ID => null);

    // Get service provider permissions for this group
    $criteria = new Criteria;
    $criteria->addJoin(QubitAclPermission::OBJECT_ID, QubitObject::ID, Criteria::LEFT_JOIN);
    $criteria->add(QubitAclPermission::GROUP_ID, $this->resource->id);
    $c1 = $criteria->getNewCriterion(QubitObject::CLASS_NAME, 'QubitServiceProvider');
    $criteria->add($c1);
    ...
    if (null !== $permissions = QubitAclPermission::get($criteria))
    {
      ...
      {
        $this->serviceProviders[$p->objectId][$p->action] = $p;
      }
    }
    ...
    if ($request->isMethod('post'))
    {
      ...
      {
        $this->processForm();
		$this->resource->objectId = QubitServiceProvider::ROOT_ID;
        $this->resource->save();
        $this->redirect(array($this->resource, 'module' => 'aclGroup', 'action' => 'indexServiceProviderAcl'));
      }
    }
  }
}
