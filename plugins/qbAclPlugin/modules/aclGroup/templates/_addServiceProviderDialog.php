...
<!-- form for yui dialog -->
<div id="myDialog">
  <div class="hd"><?php echo __('Add %1%', array('%1%' => lcfirst(sfConfig::get('app_ui_label_service_provider')))) ?></div>
  <div class="bd">
    <form name="serviceProviderAcl" method="post">
      <div class="form-item">
        <label for="addServiceProvider"><?php echo __('%1% name', array('%1%' => sfConfig::get('app_ui_label_service_provider'))) ?></label>
        <select name="addServiceProvider" id="addServiceProvider" class="form-autocomplete"></select>
        <input class="list" type="hidden" value="<?php echo url_for(array('module' => 'serviceProvider', 'action' => 'autocomplete')) ?>"/>
      </div>
    </form>
  </div>
</div>
...
var handleSubmit = function() {
  var serviceProviderInput = jQuery('input[name="addServiceProvider"]');
  var objectUri = serviceProviderInput.val().split('/');
  ...
  else if ('null' != serviceProviderInput.val())
  {
    ...
    newTable.find('caption').text(serviceProviderInput.next('input').val());
    newTable.hide();
    jQuery('a#addServiceProviderLink').parent('div').before(newTable);
    newTable.slideDown();
  }
  ...
  // Clear dialog values
  serviceProviderInput.val('null');
  serviceProviderInput.next('input').val(null);
};
var myButtons = [
...
// Focus on 'addServiceProvider' (visible) field
myDialog.showEvent.subscribe(function () {
  document.getElementById('addServiceProvider').focus();
}, this, true);
