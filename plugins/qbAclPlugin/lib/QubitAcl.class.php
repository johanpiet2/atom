...
class QubitAcl {
	...
	protected $_roles = array() , $_resources = array() , $_user;
	...
	public static function check($resource, $actions, $options = array()) {
		...
		if ('cli' == sfContext::getInstance()->getConfiguration()
			->getEnvironment()) {
			return true;
		}
	...
	*/
	private static function checkAccessByClass($resource, $user, $action, $options) {
		switch (get_class($resource)) {
			...
			case 'QubitRightsHolder':
				$hasAccess = $user->isAuthenticated() && ($user->hasGroup(QubitAclGroup::ADMINISTRATOR_ID) || $user->hasGroup(QubitAclGroup::EDITOR_ID));
			break;
	...
	public static function isAllowed($role, $resource, $action, $options = array()) {
		...
		if ('read' == $action && $resource instanceOf QubitInformationObject) {
			...
			if (QubitTerm::PUBLICATION_STATUS_DRAFT_ID == $resource->getPublicationStatus()
				->statusId) {
				$instance = self::getInstance()->buildAcl($resource, $options);

				return ($instance
					->acl
					->isAllowed($role, $resource, 'read') && $instance
					->acl
					->isAllowed($role, $resource, 'viewDraft'));
			}
		}
		...
		if (is_object($role)) {
			if (!in_array($role->id, $this->_roles)) {
				foreach ($role
					->ancestors
					->andSelf()
					->orderBy('lft') as $ancestor) {
					if (!in_array($ancestor->id, $this->_roles)) {
						$this
							->acl
							->addRole($ancestor, $ancestor->parentId);
					}
				}
			}
		}
	...
	protected function buildUserRoleList($user) {
		...
		if ($user->isAuthenticated()) {
			...
			$this
				->acl
				->addRole(QubitAclGroup::getById(QubitAclGroup::AUTHENTICATED_ID));
			$this->_roles[] = QubitAclGroup::AUTHENTICATED_ID;

			// Add groups (if user belongs to any)
			if (0 < count($aclUserGroups = $user
				->user
				->getAclUserGroups())) {
				foreach ($aclUserGroups as $aclUserGroup) {
					...
					$this
						->acl
						->addRole($aclGroup, $aclGroup->parent);
			...
			$this
				->acl
				->addRole($user->getUserID() , $parents);
			...
			$this
				->acl
				->addRole(QubitAclGroup::getById(QubitAclGroup::ANONYMOUS_ID));
	...
	protected function buildResourceList($resource, $options = array()) {
		...
		//$t = fopen("/var/www/html/atom/data.txt","a");
		//fwrite($t,">>>>>>>>>>>>>>>>>>>>3333 r->id: ".$this->ancestorsId[$i]." r->parentId: ". $this->ancestorsParentId[$i] . "<".$i."> <<<<".PHP_EOL);
		//	fwrite($t,PHP_EOL);
		//	fwrite($t,PHP_EOL);
		//fclose($t);
		...
		//SITA slow...
		// Add resource hierarchy - removed SITA - replaced with $this->buildAncestors($resourceId);
		if ($resource instanceof QubitInformationObject) {
			$sql = 'SELECT PARENT_ID FROM information_object WHERE ID = ';
			$this->buildAncestors($resourceId, $sql);
			if ((is_object($resource)) && (0 < count($this->ancestorsId))) {
				for ($i = count($this->ancestorsId) - 1;$i >= 0;$i--) {
					if (!in_array($this->ancestorsId[$i], $this->_resources)) {
						$this
							->acl
							->addResource($this->ancestorsId[$i], $this->ancestorsParentId[$i]);
						$this->_resources[] = $this->ancestorsId[$i];
					}
				}
			}
			else {
			...
			if ((is_object($resource) && 0 < count($resources = $resource
				->ancestors
				->andSelf()
				->orderBy('lft')))) {
	...
	// Slow responce on using lft and rgt. This gets all children ID's SITA JJP
	public function buildAncestors($Id, $sql) {
		if ($Id != "") {
			if (!isset($connection)) {
				$connection = Propel::getConnection();
			}
			$statement = $connection->prepare($sql . $Id);
			$statement->execute();

			$sessionRows = $statement->fetchAll();
			if (count($sessionRows) != 0) {
				$this->ancestorsId[] = $Id;
				$this->ancestorsParentId[] = $sessionRows[0][0];
				$this->buildAncestors($sessionRows[0][0], $sql);
			}
	...
	protected function buildAcl($resource, $options = array()) {
		...
		if ($this
			->_user
			->isAuthenticated()) {
			$c2 = $criteria->getNewCriterion(QubitAclPermission::USER_ID, $this
				->_user
				->getUserID());
			$c1->addOr($c2);
		}
		...
		if ((!$this
			->_user
			->isAdministrator()) && (QubitSetting::getByName('open_system') == '0')) {
			if ($resource instanceof QubitInformationObject) {
				if (null !== $repository = $resource->getRepository(array(
					'inherit' => true
				))) {
		...
		if (0 < count($permissions = QubitAclPermission::get($criteria))) {
			foreach ($permissions as $permission) {
				...
				*/
				...
				if (null != $permission->conditional || in_array($permission->action, array(
					'update',
					'translate'
				))) {
	...
	public static function forwardToLoginAction() {
		sfContext::getInstance()
			->getController()
			->forward(sfConfig::get('sf_login_module') , sfConfig::get('sf_login_action'));
	...
	public static function getFilterCriterion($criteria, $root, $action) {
		...
		if (0 < count($permissions)) {
			foreach ($permissions as $permission) {
				switch ($action) {
					case 'createTerm':
						if (null !== $slug = $permission->getConstants(array(
							'name' => 'taxonomy'
						))) {
					...
					case 'viewDraft':
						if (null !== $repository = $permission->getConstants(array(
							'name' => 'repository'
						))) {
							$repoId = QubitPdo::fetchColumn('SELECT object_id FROM slug WHERE slug=?', array(
								$repository
							));
	...
	private static function getAllowedResourcesCriterion($criteria, $rootClass, $resourceCache, $resourceIds, $allow) {
		while ($resourceId = array_shift($resourceIds)) {
			...
			if (1 == ($resource->rgt - $resource->lft)) {
				$criterion = $criteria->getNewCriterion(constant("$rootClass::ID") , $resourceId, $allow ? Criteria::EQUAL : Criteria::NOT_EQUAL);
			}
			else
			// Else, include object and all children
			{
				$criterion = $criteria->getNewCriterion(constant("$rootClass::LFT") , $resource->lft, $allow ? Criteria::GREATER_EQUAL : Criteria::LESS_EQUAL);

				$criterion2 = $criteria->getNewCriterion(constant("$rootClass::RGT") , $resource->rgt, $allow ? Criteria::LESS_EQUAL : Criteria::GREAT_EQUAL);
