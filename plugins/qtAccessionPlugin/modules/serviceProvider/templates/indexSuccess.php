...
<?php slot('title') ?>
  <h1 class="multiline">
    <?php echo __('View Service Provider') ?>
    ...
    <div>
      <ul>
        <?php foreach (QubitRelation::getRelationsByObjectId($resource->id, array('typeId' => QubitTerm::DONOR_ID)) as $item): ?>
          <li><?php echo link_to(render_title($item->subject), array($item->subject, 'module' => 'serviceProvider')) ?></li>
        <?php endforeach; ?>
      </ul>
    </div>
...
<?php slot('after-content') ?>
  <section class="actions">
    <ul>            
      <?php if (QubitAcl::check($serviceprovider, 'update') || sfContext::getInstance()->getUser()->isAdministrator()): ?>
        <li><?php echo link_to(__('Edit'), array($resource, 'module' => 'serviceProvider', 'action' => 'update'), array('title' => __('Edit'), 'class' => 'c-btn')) ?></li>
      <?php endif; ?>
      <?php if (QubitAcl::check($serviceprovider, 'delete') || sfContext::getInstance()->getUser()->isAdministrator()): ?>
        <li><?php echo link_to(__('Delete'), array($resource, 'module' => 'serviceProvider', 'action' => 'delete', 'source' => $resource->id), array('title' => __('Delete'), 'class' => 'c-btn c-btn-delete')) ?></li>
      <?php endif; ?>
      ...
      <li><?php echo link_to(__('Back to list'), array('module' => 'serviceProvider', 'action' => 'browse'), array('title' => __('Back to list'), 'class' => 'c-btn')) ?></li>
      <?php endif; ?>
    </ul>
  </section>
<?php end_slot() ?>
