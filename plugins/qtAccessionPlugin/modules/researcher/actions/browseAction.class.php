 ...
 *
 * Show paginated list of researchers.
 *
...
class researcherBrowseAction extends sfAction
{
    public function execute($request)
    {
        ...
        //do not get root researcher - one instance
        $criteria->add(QubitResearcher::ID, 20, Criteria::NOT_EQUAL);
        ...
        if ((!$this->context->user->isAdministrator()) && (QubitSetting::getByName('open_system') == '0')) {
            ...
            if (0 < count($userRepos = $repositories->getRepositoriesById($this->context->user->getAttribute('user_id')))) {
                ...
                foreach ($userRepos as $userRepo) {
                    $criteria->addOR(QubitResearcher::REPOSITORY_ID, $userRepo, Criteria::EQUAL);
                }
            } else {
                // if not logged in do not show any repositories
                $criteria->add(QubitResearcher::REPOSITORY_ID, '0000', Criteria::EQUAL);
            }
        }
