...
/**
 * Import an XML document of Researcher into Qubit.
 *
...
class XmlImportResearcher
{
  ...
  public function import($xmlFile, $options = array(), $type)
  {
	...
	{
   		throw new sfError404Exception('Unable to parse XML file: Perhaps not a Researcher import file');
    }
          ...
          case 'researcherId':
            foreach ($nodeList2 as $item)
            {
              if (($childNode = $importDOM->xpath->query('.', $item)) !== null)
              {
    			...
    			$currentObject->actorImportId = $childNode->item(0)->nodeValue;
              }
			}

            break;

          case 'researcherAddress':
		      if (($childNode = $importDOM->xpath->query('.', $item)) !== null)
		      {
		        foreach ($nodeList2 as $item)
		        {
