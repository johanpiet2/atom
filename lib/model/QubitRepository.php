...
class QubitRepository extends BaseRepository
{
    ...
    public function updateRelatedIos()
    {
        ...
        foreach ($this->informationObjects as $io) {
            ...
            if (in_array($env, array(
                ...
                QubitSearch::getInstance()->update($io, array(
        ...
        $jobsUrl = $context->routing->generate(null, array(
            'module' => 'jobs',
            'action' => 'browse'
        ));
        $message = $context->i18n->__('Your repository has been updated. Its related descriptions are being updated asynchronously – check the <a href="%1">job scheduler page</a> for status and details.', array(
            '%1' => $jobsUrl
        ));
    ...
    ***************/
    ...
    /**
     * Show only Repositry list linked to user - JJP SITA One Instance
     *
     * Administrator can see all 
     * Open system can see all 
     *
     * @return array of repository name and id linked to logged in user
     * 
     */
    public static function filteredUser($userId, $isAdmin)
    {
        $choices = array();
        if (($isAdmin == 0) && (QubitSetting::getByName('open_system') == '0')) {
            ...
            foreach (QubitRepository::getAll() as $item) {
                if ($item->__toString() != "") {
                    if (0 < count($userRepos = $repositories->getRepositoriesById($userId))) {
    ...
    /**
     * Show only Repositories linked to user - JJP SITA One Instance
     *
     * Administrator can see all 
     * Open system can see all 
     *
     * @return array of repositories linked to logged in user
     * 
     */
    public static function filteredUserRepositories($userId, $isAdmin)
    {
        $repositories = array();
        if (($isAdmin == 0) && (QubitSetting::getByName('open_system') == '0')) {
	        $repos = new QubitUser;
	        
	        $repositoryCount = count($userRepos = $repos->getRepositoriesById($userId));
            if (0 < count($repositoryCount)) {
            	$userRepos = $repos->getRepositoriesById($userId);
                for ($x = 0; $x <= $repositoryCount-1; $x++) {
					$getRepo = QubitRepository::getById($userRepos[$x]);
