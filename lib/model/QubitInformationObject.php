 ...
 * @author Johan Pieterse, State Information Technology Agency <johan.pieterse@sita.co.za>
 */
class QubitInformationObject extends BaseInformationObject
{
    ...
    public function __get($name)
    {
        ...
        switch ($name) {
            ...
            case 'scriptOfDescription':
                ...
                if (isset($this->values[$name]) && null !== $value = unserialize($this->values[$name]->__get('value', $options + array(
                    'sourceCulture' => true
                )))) {
                    return $value;
                }
    ...
    public function __set($name, $value)
    {
        ...
        switch ($name) {
            ...
            case 'scriptOfDescription':
                ...
                $this->values[$name]->__set('value', serialize($value), $options + array(
                    'sourceCulture' => true
                ));
    ...
    public function save($connection = null)
    {
        ...
        $userActionId = ($this->new) ? QubitTerm::USER_ACTION_CREATION_ID : QubitTerm::USER_ACTION_MODIFICATION_ID;
        ...
        foreach ($this->digitalObjectsRelatedByobjectId as $item) {
            ...
            $item->object = $this;
            $item->save($connection);
			if (QubitSetting::getByName('multi_digital_linked') == '0') {
				break; // Save only one digital object per information object - SITA create multiple attachments
			}
        }
        ...
        if ($this->indexOnSave) {
            QubitSearch::getInstance()->update($this, array(
    ...
    public function delete($connection = null)
    {
        ...
        $relations = QubitRelation::getRelationsByObjectId($this->id, array(
    ...
    public function urlForEadExport()
    {
        if ($this->getPublicationStatus()->statusId == QubitTerm::PUBLICATION_STATUS_PUBLISHED_ID && file_exists($this->pathToEadExport())) {
            ...
            return sfContext::getInstance()->routing->generate(null, array(
                $this,
                'module' => 'sfEadPlugin',
                'sf_format' => 'xml'
            ));
    ...
    ***************************/
    ...
    public function getFindingAidStatus()
    {
        ...
        return $property->getValue(array(
            'sourceCulture' => true
        ));
    }
    ...
    ";
        ...
        $rows = QubitPdo::fetchAll($sql, array(
            $this->lft,
            $this->rgt,
            $culture,
            $level
    ...
    ************************/
    ...
    {
        return $this->getActors($options = array(
    ...
    {
        return $this->getActors($options = array(
            'eventTypeId' => QubitTerm::PUBLICATION_ID
        ));
    }
    ...
    {
        return $this->getActors($options = array(
            'eventTypeId' => QubitTerm::CONTRIBUTION_ID
        ));
    }
    ...
    public function getDates(array $options = array())
    {
        ...
        $criteria->addMultipleJoin(array(
            ...
            array(
                QubitEvent::SOURCE_CULTURE,
                QubitEventI18n::CULTURE
            )
        ), Criteria::LEFT_JOIN);
    ...
    ******************************************/
        ...
        if ($this->getCreators()) {
            ...
            foreach ($creators as $creator) {
                ...
                foreach ($creator->getOtherNames() as $otherName) {
                    $creatorNameString .= $otherName->getName(array(
                        'culture' => $culture
                    )) . ' ';
                }
            }
    ...
    public function getCreatorsHistoryString($options = array())
    {
        if ($this->getCreators()) {
            ...
            $creators             = $this->getCreators();
            foreach ($creators as $creator) {
                $creatorHistoryString .= $creator->getHistory(array(
                    'culture' => $culture
                )) . ' ';
            }
    ...
    public function getDatesString($options = array())
    {
        if ($this->getDates()) {
            ...
            foreach ($dates as $date) {
                $datesString .= $date->getDate(array(
                    'culture' => $culture
                )) . ' ';
            }
    ...
    public function getNameAccessPointsString($options = array())
    {
        ...
        foreach ($this->nameAccessPoints = QubitRelation::get($criteria) as $name) {
            $nameAccessPointString .= $name->object->getAuthorizedFormOfName(array(
                'culture' => $culture
            )) . ' ';
        }
    ...
    *********************/
    ...
    {
        $relatedPhysicalObjects = QubitRelation::getRelatedSubjectsByObjectId('QubitPhysicalObject', $this->id, array(
            'typeId' => QubitTerm::HAS_PHYSICAL_OBJECT_ID
        ));
    ...
    ******************/
    ...
    /******************
    Digital Objects SITA batch export/publish
    ******************/
    ...
    public function getDigitalObjectPublicUrl()
    {
        ...
        if (!QubitAcl::check($this, 'readMaster') && null !== $do->reference && QubitAcl::check($this, 'readReference')) {
    ...
    *********************/
     ...
     * @return QubitInformationObject this object
     */
    public function addBookoutObject($bookoutObject)
    {
        if (get_class($bookoutObject) == 'QubitBookoutObject' && $this->getBookoutObject($bookoutObject->id) === null) {
            $relation = new QubitRelation;
            $relation->setSubject($bookoutObject);
            $relation->setTypeId(QubitTaxonomy::BOOKOUT_TYPE_ID);
            $this->relationsRelatedByobjectId[] = $relation;
        }
    ...
    public function getBookoutObject($bookoutObjectId)
    {
        $criteria = new Criteria;
        $criteria->add(QubitRelation::OBJECT_ID, $this->id);
        $criteria->add(QubitRelation::SUBJECT_ID, $bookoutObjectId);
        $criteria->add(QubitRelation::TYPE_ID, QubitTaxonomy::BOOKOUT_TYPE_ID);
        return QubitRelation::getOne($criteria);
    }
    
    public function getBookoutObjects()
    {
        $relatedBookoutObjects = QubitRelation::getRelatedSubjectsByObjectId('QubitBookoutObject', $this->id, array(
            'requestorId' => QubitTaxonomy::BOOKOUT_TYPE_ID
        ));
        return $relatedBookoutObjects;
    }
    
    protected function deleteBookoutObjectRelations()
    {
        $relations = QubitRelation::getRelationsByObjectId($this->id, array(
            'requestorId' => QubitTaxonomy::BOOKOUT_TYPE_ID
        ));
    ...
    bookin Objects
    *********************/
    public function addBookinObject($bookinObject)
    {
        if (get_class($bookinObject) == 'QubitBookinObject' && $this->getBookinObject($bookinObject->id) === null) {
            $relation = new QubitRelation;
            $relation->setSubject($bookinObject);
            $relation->setTypeId(QubitTaxonomy::BOOKIN_TYPE_ID);
            $this->relationsRelatedByobjectId[] = $relation;
        }
    ...
    public function getBookinObject($bookinObjectId)
    {
        $criteria = new Criteria;
        $criteria->add(QubitRelation::OBJECT_ID, $this->id);
        $criteria->add(QubitRelation::SUBJECT_ID, $bookinObjectId);
        $criteria->add(QubitRelation::TYPE_ID, QubitTaxonomy::BOOKIN_TYPE_ID);
        return QubitRelation::getOne($criteria);
    }
    
    public function getBookinObjects()
    {
        $relatedBookinObjects = QubitRelation::getRelatedSubjectsByObjectId('QubitBookinObject', $this->id, array(
            'requestorId' => QubitTaxonomy::BOOKOUT_TYPE_ID
        ));
        return $relatedBookinObjects;
    }
    
    protected function deleteBookinObjectRelations()
    {
        $relations = QubitRelation::getRelationsByObjectId($this->id, array(
            'requestorId' => QubitTaxonomy::BOOKIN_TYPE_ID
        ));
        foreach ($relations as $relation) {
            $relation->delete();
        }
    ...
    *********************/
    /**
     * Add a relation from this info object to a Access object. Check to make
     * sure the relationship is unique.
     *
     * @param QubitAccessObject $accessObject Subject of relationship
     * @return QubitInformationObject this object
     */
    public function addAccessObject($accessObject)
    {
        if (get_class($accessObject) == 'QubitAccessObject' && $this->getAccessObject($accessObject->id) === null) {
            $relation = new QubitRelation;
            $relation->setSubject($accessObject);
            $relation->setTypeId(QubitTaxonomy::ACCESS_TYPE_ID);
            $this->relationsRelatedByobjectId[] = $relation;
        }
    ...
    public function getAccessObject($accessObjectId)
    {
        $criteria = new Criteria;
        $criteria->add(QubitRelation::OBJECT_ID, $this->id);
        $criteria->add(QubitRelation::SUBJECT_ID, $accessObjectId);
    ...
    {
        $relatedAccessObjects = QubitRelation::getRelatedSubjectsByObjectId('QubitAccessObject', $this->id, array(
            'typeId' => QubitTaxonomy::ACCESS_TYPE_ID
        ));
        return $relatedAccessObjects;
    }
    
    protected function deleteAccessObjectRelations()
    {
        $relations = QubitRelation::getRelationsByObjectId($this->id, array(
            'typeId' => QubitTaxonomy::ACCESS_TYPE_ID
        ));
    ...
    *********************/
     ...
     * @return QubitInformationObject this object
     */
    public function addPresevationObject($presevationObject)
    {
        if (get_class($presevationObject) == 'QubitPresevationObject' && $this->getPresevationObject($presevationObject->id) === null) {
            $relation = new QubitRelation;
            $relation->setSubject($presevationObject);
            $relation->setTypeId(QubitTaxonomy::PRESERVATION_TYPE_ID);
            $this->relationsRelatedByobjectId[] = $relation;
        }
    ...
    public function getPresevationObject($presevationObjectId)
    {
        $criteria = new Criteria;
        $criteria->add(QubitRelation::OBJECT_ID, $this->id);
        $criteria->add(QubitRelation::SUBJECT_ID, $presevationObjectId);
        $criteria->add(QubitRelation::TYPE_ID, QubitTaxonomy::PRESERVATION_TYPE_ID);
        return QubitRelation::getOne($criteria);
    }
    
    public function getPresevationObjects()
    {
        $relatedPresevationObjects = QubitRelation::getRelatedSubjectsByObjectId('QubitPresevationObject', $this->id, array(
            'typeId' => QubitTaxonomy::PRESERVATION_TYPE_ID
        ));
        return $relatedPresevationObjects;
    }
    
    protected function deletePresevationObjectRelations()
    {
        $relations = QubitRelation::getRelationsByObjectId($this->id, array(
            'typeId' => QubitTaxonomy::PRESERVATION_TYPE_ID
        ));
    ...
    *****************/
    ...
    //  public function setRepositoryAddress($address, $streetAddress, $suburb, $city, $province, $country, $postalCode, $phone, $email, $website, $cell, $fax, $contactPerson, $latitude, $longitude, $postalAddress, $postalSuburb, $postalCity, $postalProvince, $postalCountry, $postalPostCode)
        ...
        if ($repository = $this->getRepository()) {
            if ($primaryContact = $repository->getPrimaryContact()) {
                if (is_null($primaryContact->getStreetAddress())) {
                    $primaryContact->setStreetAddress($streetAddress);
                    $primaryContact->setCity($city);
                    $primaryContact->setRegion($province);
                    
                    // $primaryContact->setCountry($country);
                    
                    $primaryContact->setPostalCode($postalCode);
                    $primaryContact->setTelephone($phone);
                    $primaryContact->setEmail($email);
                    $primaryContact->setWebsite($website);
                    
                    // jjp SITA 17 Dec 2014 - Added Cell number
                    
                    $primaryContact->setCell($cell);
                    $primaryContact->setFax($fax);
                    $primaryContact->setContactPerson($contactPerson);
                    $primaryContact->setLatitude($latitude);
                    $primaryContact->setLongitude($longitude);
                    
                    // jjp SITA 17 Dec 2014 - Added Postal Address
                    
                    $primaryContact->setPostalAddress($postalAddress);
                    $primaryContact->setPostalCity($postalCity);
                    $primaryContact->setPostalRegion($postalProvince);
                    $primaryContact->setPostalPostCode($postalPostCode);
                    $primaryContact->save();
                }
            } else {
                ...
                $contactInformation->setStreetAddress($streetAddress);
                $contactInformation->setCity($city);
                $contactInformation->setRegion($province);
                
                // $primaryContact->setCountry($country);
                
                $contactInformation->setPostalCode($postalCode);
                $contactInformation->setTelephone($phone);
                $contactInformation->setEmail($email);
                $contactInformation->setWebsite($website);
                $contactInformation->setCell($cell);
                $contactInformation->setFax($fax);
                $contactInformation->setContactPerson($contactPerson);
                $contactInformation->setLatitude($latitude);
                $contactInformation->setLongitude($longitude);
                ...
                $contactInformation->setPostalCity($postalCity);
                $contactInformation->setPostalRegion($postalProvince);
                $contactInformation->setPostalPostCode($postalPostCode);
    ...
    public function setActorByName($name, $options)
    {
        ...
        if ($actor && !empty($options['history']) && $actor->history !== $options['history']) {
            ...
            $actor = QubitActor::getByAuthorizedFormOfName($name, array(
    ...
    public function importLangusageEadData($langusageNode)
    {
        ...
        if (count($languagesOfDescription)) {
            $this->addProperty('languageOfDescription', serialize($languagesOfDescription));
        }
        
        // add script(s) of description, if any
        if (count($scriptsOfDescription)) {
            $this->addProperty('scriptOfDescription', serialize($scriptsOfDescription));
        }
    }
    ...
    public function importDescLanguse($langusageNode, $langList)
    {
        $languagesOfDescription = array();
        $scriptsOfDescription   = array();
        $langCodeConvertor      = new fbISO639_Map;
        ...
        QubitXMLImport::addLog("importDescLanguse - langList: " . $langList, "", get_class($this), false);
        // get language nodes
        if (0 < strlen($langCode = $langusageNode->getAttribute('langcode'))) {
            QubitXMLImport::addLog("langCode: " . $langCode, "", get_class($this), false);
            // get all as language(s) of description
            if ($langCode != "NoCode") // SITA JJP
                {
                if ($langCode2 = $langCodeConvertor->getID1($langCode, false)) {
                    QubitXMLImport::addLog("languagesOfDescription langCode: " . $langList . "|" . strtolower($langCode2), "", get_class($this), false);
                    if ($langList != "") {
                        return $langList . "|" . strtolower($langCode2);
                    } else {
                        return strtolower($langCode);
                    }
                } else {
                    QubitXMLImport::addLog("languagesOfDescription key: " . $langList . "|" . strtolower($langCode), "", get_class($this), false);
                    if ($langList != "") {
                        return $langList . "|" . strtolower($langCode);
                    } else {
                        return strtolower($langCode);
                    }
                }
                ...
                foreach ($language_en as $key => $valueLanguage) {
                    if ($langusageNode->nodeValue == $valueLanguage) {
                        if ($langCode = $langCodeConvertor->getID1($key, false)) {
                            QubitXMLImport::addLog("languagesOfDescription langCode: " . $langList . "|" . strtolower($langCode), "", get_class($this), false);
                            if ($langList != "") {
                                return $langList . "|" . strtolower($langCode);
                            } else {
                                return strtolower($langCode);
                            ...
                            QubitXMLImport::addLog("languagesOfDescription key: " . $langList . "|" . strtolower($key), "", get_class($this), false);
                            if ($langList != "") {
                                return $langList . "|" . strtolower($key);
                            } else {
            ...
            QubitXMLImport::addLog("langCode: Empty-" . $langList, "", get_class($this), true);
            return $langList;
        }
    }
    ...
    public function importDescLanguseScript($langusageNode, $langList)
    {
        $languagesOfDescription = array();
        $scriptsOfDescription   = array();
        $langCodeConvertor      = new fbISO639_Map;
        ...
        QubitXMLImport::addLog("importDescLanguseScript key: " . $langusageNode->nodeValue, "", get_class($this), true);
        
        // get language nodes
        if (0 < strlen($langCode = $langusageNode->getAttribute('scriptcode'))) {
            // get all as language(s) of description
            if ($langCode != "NoCode") // SITA JJP
                {
                if ($langCode2 = $langCodeConvertor->getID1($langCode, false)) {
                    if ($langList != "") {
                        return $langList . "|" . $langCode2;
                    } else {
                        return $langCode2;
                    }
                } else {
                ...
                $c_en              = sfCultureInfo::getInstance();
                $language_en       = $c_en->getLanguages();
                QubitXMLImport::addLog("importDescLanguseScript key2: " . $langusageNode->nodeValue, "", get_class($this), true);
                foreach ($language_en as $key => $valueLanguage) {
                    if ($langusageNode->nodeValue == $valueLanguage) {
                        if ($langCode = $langCodeConvertor->getID1($key, false)) {
                            if ($langList != "") {
            ...
            QubitXMLImport::addLog("importDescLanguseScript key: Empty", "", get_class($this), false);
            return $langList;
        }
    }
    ...
    public function importScritpLanguse($langusageNode, $langList)
    {
        $languagesOfDescription = array();
        $scriptsOfDescription   = array();
        $langCodeConvertor      = new fbISO639_Map;
        $sf_user                = sfContext::getInstance()->user;
        QubitXMLImport::addLog("importScritpLanguse - langusageNode->nodeValue: " . $langusageNode->nodeValue . "|" . strtolower($langCode), "", get_class($this), false);
        
        // get language nodes
        if (0 < strlen($langCode = $langusageNode->getAttribute('scriptcode'))) {
            QubitXMLImport::addLog("importScritpLanguse - langCode scriptcode: " . $langCode . "|" . strtolower($langCode), "", get_class($this), false);
            // get all as language(s) of description
            if ($langCode != "NoCode") // SITA JJP
                {
                if ($langCode2 = $langCodeConvertor->getID1($langCode, false)) {
                    array_push($languagesOfDescription, strtolower($langCode2));
                } else {
                    array_push($languagesOfDescription, strtolower($langCode));
                }
            } else {
                $langCodeConvertor = new fbISO639_Map;
                $c_en              = sfCultureInfo::getInstance();
                $language_en       = $c_en->getScripts();
                foreach ($language_en as $key => $valueLanguage) {
                    if ($langusageNode->nodeValue == $valueLanguage) {
                        if ($langCode = $langCodeConvertor->getID1($key, false)) {
                            QubitXMLImport::addLog("langscriptcode scriptcode: " . $langList . "|" . strtolower($langCode), "", get_class($this), false);
                            if ($langList != "") {
                                return $langList . "|" . ($langCode);
                            } else {
                                return ($langCode);
                            ...
                            QubitXMLImport::addLog("langscriptcode key: " . $langList . "|" . strtolower($key), "", get_class($this), false);
                            if ($langList != "") {
                                return $langList . "|" . ($key);
                            } else {
                                return ($key);
            ...
            QubitXMLImport::addLog("importScritpLanguse: No script ", "", get_class($this), false);
        }
    }
    ...
    public function importMaterialLanguse($langusageNode, $langList)
    {
        $languagesOfDescription = array();
        $scriptsOfDescription   = array();
        $langCodeConvertor      = new fbISO639_Map;
        $sf_user                = sfContext::getInstance()->user;
        QubitXMLImport::addLog("importMaterialLanguse - langusageNode->nodeValue: " . $langusageNode->nodeValue . " " . $langusageNode->getAttribute('langcode') . "|" . strtolower($langCode), "", get_class($this), true);
        
        // get language nodes
        if (0 < strlen($langCode = $langusageNode->getAttribute('langcode'))) {
            QubitXMLImport::addLog("langCode scriptcode: " . $langCode . "|" . strtolower($langCode), "", get_class($this), true);
            // get all as language(s) of description
            if ($langCode != "NoCode") // SITA JJP
                {
                if ($langCode2 = $langCodeConvertor->getID1($langCode, false)) {
                ...
                foreach ($language_en as $key => $valueLanguage) {
                    if ($langusageNode->nodeValue == $valueLanguage) {
                        QubitXMLImport::addLog("languagesOfDescription key: " . $key . "|" . strtolower($valueLanguage), "", get_class($this), true);
                        if ($langCode = $langCodeConvertor->getID1($key, false)) {
                            QubitXMLImport::addLog("languagesOfDescription scriptcode: " . $langList . "|" . strtolower($langCode), "", get_class($this), true);
                            if ($langList != "") {
                                return $langList . "|" . ($langCode);
                            } else {
                                return ($langCode);
                            ...
                            QubitXMLImport::addLog("languagesOfDescription key: " . $langList . "|" . strtolower($key), "", get_class($this), true);
                            if ($langList != "") {
                                return $langList . "|" . ($key);
                            } else {
                                return ($key);
            ...
            QubitXMLImport::addLog("importMaterialLanguse: No material language ", "", get_class($this), true);
        }
    }
    ...
    public function importLanguageEadData($langusageNode, $langType)
    {
        // add language(s) of description, if any
        $options = array();
        if (langusageNode != "") {
            QubitXMLImport::addLog("importLanguageEadData - langusageNode: " . ($langusageNode), "", get_class($this), true);
            $languagesOfDescription = explode("|", $langusageNode);
            $languagesOfDescription = array_unique($languagesOfDescription, SORT_REGULAR);
            QubitXMLImport::addLog("importLanguageEadData - count($langType)" . count($languagesOfDescription), "", get_class($this), true);
            
            if (null != $this->id && QubitProperty::isExistent($this->id, 'languageOfDescription', serialize($languagesOfDescription), $options)) {
                return;
            }
            $lDesc = serialize($languagesOfDescription);
            QubitXMLImport::addLog("2. serialize($langType)>>>>" . $lDesc . "<<<<", "", get_class($this), true);
            $property = new QubitProperty;
            $property->setName($langType);
            $property->setValue($lDesc, $options);
            if (isset($options['scope'])) {
                $property->setScope($options['scope']);
            }
            ...
            try {
                ...
                QubitXMLImport::addLog("Error message: " . $e->getMessage(), "", get_class($this), true);
            }
    ...
    /**
     * Import language-related data from a <languXmlImportArchivalDescriptionsage> tag in EAD2002
     *
     * @param $langusageNode  DOMNode  EAD langusage DOM node
     *
     * Get per scriptcode
     ...
     * SITA JJP
     */
    public function importLanguageData($langusageNode, $langType, $objectId)
    {
        // add language(s) of description, if any
        $options = array();
        QubitXMLImport::addLog("importLanguageData - langusageNode: " . ($langusageNode), "", get_class($this), true);
        if (langusageNode != "") {
            
            $languagesOfDescription = explode("|", $langusageNode);
            $languagesOfDescription = array_unique($languagesOfDescription, SORT_REGULAR);
            
            QubitXMLImport::addLog("count($langType)" . count($languagesOfDescription), "", get_class($this), true);
            
            $lDesc = serialize($languagesOfDescription);
            QubitXMLImport::addLog("1. serialize($langType)>>>>" . $lDesc . "<<<<", "", get_class($this), true);
            //first see if it exist
            if (null == $fProperty = QubitProperty::getOneByObjectIdAndName($objectId, $langType)) {
                $property = new QubitProperty;
                $property->setName($langType);
                $property->setValue($lDesc, $options);
                if (isset($options['scope'])) {
                ...
                QubitXMLImport::addLog("1.2. serialize($langType)>>>>" . $lDesc . "<<<<", "", get_class($this), true);
                try {
                    $fProperty->setValue($lDesc, $options);
                    $fProperty->save();
                }
                catch (Exception $e) {
                    QubitXMLImport::addLog("Error message: " . $e->getMessage(), "", get_class($this), true);
        ...
        foreach ($entityTypes as $type => $typeId) {
            ...
            if ($nameNodes->length) {
                foreach ($nameNodes as $node) {
                    $actor = $this->setActorByName($node->nodeValue, array(
    ...
    public function importBioghistEadData($biogHistNode, $key)
    {
        ...
        if (strlen($history = QubitXmlImport::normalizeNodeValue($biogHistNode)) > 0) {
            ...
            //   - Do nothing
            if (!isset($creators[$key]) || (isset($creators[$key]->history) && $creators[$key]->history !== $history)) {
                $actor           = new QubitActor;
                $actor->parentId = QubitActor::ROOT_ID;
    ...
    {
        $this->setTermIdPropertyUsingTermName('descriptionDetailId', $name, QubitTaxonomy::DESCRIPTION_DETAIL_LEVEL_ID);
    }
    ...
    {
        $this->setTermIdPropertyUsingTermName('descriptionStatusId', $name, QubitTaxonomy::DESCRIPTION_STATUS_ID);
    }
    ...
    public function importPhysicalObject($location, $name, $options = array())
    {
        ...
        $term = ($fullType) ? QubitFlatfileImport::createOrFetchTerm(QubitTaxonomy::PHYSICAL_OBJECT_TYPE_ID, $fullType) : false;
        
        // Check for an existing physical object within this collection with the same name
        if (isset($options['collectionId'])) {
            $object = QubitPhysicalObject::checkPhysicalObjectExistsInCollection($name, $location, ($term) ? $term->id : null, $options['collectionId']);
        }
    ...
    ***************/
    ...
    public static function getRecordByOaiID($oai_local_identifier)
    {
        ...
        return QubitInformationObject::get($criteria)->offsetGet(0, array(
            'defaultValue' => null
        ));
    }
    ...
    public function getOaiIdentifier()
    {
        ...
        $oaiRepositoryCode = QubitSetting::getByName('oai_repository_code')->getValue(array(
            'sourceCulture' => true
        ));
    ...
    {
        $this->addProperty('source_oai_identifier', $value, $options = array(
            'scope' => 'oai',
            'sourceCulture' => true
        ));
    }
    ...
    *****************************************************/
    ...
    {
        return $this->getStatus($options = array(
            'typeId' => QubitTerm::STATUS_TYPE_PUBLICATION_ID
        ));
    }
    ...
    {
        return $this->setStatus($options = array(
            'statusId' => $value,
            'typeId' => QubitTerm::STATUS_TYPE_PUBLICATION_ID
        ));
    }
    ...
    *****************************************************/
    ...
    public function getTreeViewChildren(array $options = array(), &$childrenRemaining = null)
    {
        ...
        $items = array();
        if (isset($firstChild)) {
            $firstChildSiblings = $firstChild->getTreeViewSiblings(array(
                'limit' => $numberOfPreviousOrNextSiblings + 1,
                'position' => 'next'
            ), $childrenRemaining);
        ...
        do {
            ...
            switch ($sort) {
                case 'identifierTitle':
                    ...
                    $concatCurrent .= $current->getTitle(array(
                        'sourceCulture' => true
                    ));
                    ...
                    if ('next' == $position) {
                        $criteria->add('title', 'CONVERT(CONCAT(
                ...
                case 'title':
                    ...
                    if ('next' == $position) {
                        $criteria->add('title', 'CONVERT(CONCAT(COALESCE((CASE WHEN (current.TITLE IS NOT NULL AND current.TITLE <> "") THEN current.TITLE ELSE source.TITLE END), ""), LPAD(lft, 12, 0)), CHAR)
                > ' . $concatCurrent, Criteria::CUSTOM);
    ...
    public function getDigitalObjectLink()
    {
        ...
        $isText        = in_array($digitalObject->mediaTypeId, array(
            QubitTerm::TEXT_ID
        ));
        ...
        if (QubitGrantedRight::checkPremis($this->id, 'readMaster') && ($hasReadMaster || $isText)) {
            if (QubitTerm::EXTERNAL_URI_ID == $digitalObject->usageId) {
                ...
                return $request->getUriPrefix() . $request->getRelativeUrlRoot() . $digitalObject->getFullPath();
            }
        }
    }
    ...
    private function generateSlug()
    {
        ...
        switch ($slugBasis) {
            ...
            case QubitSlug::SLUG_BASIS_TITLE:
                $stringToSlugify = $this->getTitle(array(
                    'sourceCulture' => true
                ));
                break;
    ...
    {
        $criteria = new Criteria;
        return QubitRegistry::getOne($criteria);
    }
	 ...
	 * SITA
     */
    public function getRegistryById($registryId)
    {
        $criteria = new Criteria;
        $criteria->add(QubitRegistry::ID, $registryId, Criteria::EQUAL);
        return QubitRegistry::getOne($criteria);
    }
    ...
    public static function generateIdentiferFromMask()
    {
        ...
        $counterValue = $counter->getValue(array(
            'sourceCulture' => true
        ));
