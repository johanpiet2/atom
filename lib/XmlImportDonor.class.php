...
/**
 * Import an XML document of Donor into Qubit.
 *
...
class XmlImportDonor
{
  ...
  public function import($xmlFile, $options = array(), $type)
  {
	...
	{
   		throw new sfError404Exception('Unable to parse XML file: Perhaps not a Donor import file');
    }
  ...
  private function populateObject(&$domNode, &$importDOM, &$mapping, &$currentObject, $importSchema)
  {
	...
	// add logfile
	QubitXMLImport::addLog($currentObject->authorizedFormOfName, " start", true);
	...
	if ($this->importID != "")
	{
		...
		if (isset($updateObject))
		{
			...
			$updateObject->corporateBodyIdentifiers = $this->importID;
		...
		else
		{
			...
			$currentObject->corporateBodyIdentifiers = $this->importID;
    ...
    foreach ($methods as $name => $methodMap)
    {
      ...
      if (is_object($nodeList2))
      {
        switch($name)
        {
          ...
          case 'donorId':
            foreach ($nodeList2 as $item2)
            {
             if (($childNode = $importDOM->xpath->query('.', $item2)) !== null)
              {
          ...
          case 'donorAddress':
		      if (($childNode = $importDOM->xpath->query('.', $item)) !== null)
		      {
		        foreach ($nodeList2 as $item)
		        {
		          ...
		          {
				      $title = $childNode->item(0)->nodeValue.trim();
		          }
