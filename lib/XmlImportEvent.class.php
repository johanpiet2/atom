  ...
  private function populateObject(&$domNode, &$importDOM, &$mapping, &$currentObject, $importSchema)
  {
	...
	if ($this->importID != "")
	{
		...
		if (isset($updateObject))
		{
			...
			else if (isset($this->repositoryValues))
			{
				$updateObject->authorizedFormOfName = $this->authorityValues['authorizedFormOfName'];
				$updateObject->identifier = $this->authorityValues['identifier'];
				$updateObject->openingTimes = $this->authorityValues['openingTimes'];
				$updateObject->geoculturalContext = $this->authorityValues['geoculturalContext'];
				$updateObject->internalStructures = $this->authorityValues['internalStructures'];
				$updateObject->holdings = $this->authorityValues['holdings'];
				$updateObject->findingAids = $this->authorityValues['findingAids'];
				$updateObject->uploadLimit = $this->authorityValues['uploadLimit'];
				$updateObject->collectingPolicies = $this->authorityValues['collectingPolicies'];
				$updateObject->buildings = $this->authorityValues['buildings'];
				$updateObject->accessConditions = $this->authorityValues['accessConditions'];
				$updateObject->disabledAccess = $this->authorityValues['disabledAccess'];
				$updateObject->researchServices = $this->authorityValues['researchServices'];
				$updateObject->reproductionServices = $this->authorityValues['reproductionServices'];
				$updateObject->publicFacilities = $this->authorityValues['publicFacilities'];
				$updateObject->descInstitutionIdentifier = $this->authorityValues['descInstitutionIdentifier'];
				$updateObject->descRules = $this->authorityValues['descRules'];
				$updateObject->descSources = $this->authorityValues['descSources'];
				$updateObject->descRevisionHistory = $this->authorityValues['descRevisionHistory'];
			}
			...
			$this->addUpdateContact($updateObject->id,$this->addressValues);
		}
		else
		{
			// jjp SITA 07 Jan 2015 - Add import ID
			...
			// jjp SITA 06 Jan 2015 - Add contact info
			$this->addUpdateContact($currentObject->id,$this->addressValues);
		}
	}
	else
	{
        ...
        switch($name)
        {
          ...
          case 'authorityDetail':
			...
			if (($childNode = $importDOM->xpath->query('.', $item)) !== null)
			{
				foreach ($nodeList2 as $item)
			    {
			      ...
			      {
				  	  $currentObject->entityTypeId = $childNode->item(0)->nodeValue;
			      }
			      ...
			      if (($childNode = $importDOM->xpath->query('./corporateBodyIdentifiers', $item)) !== null)
			      {
				  	...
				  	$this->authorityValues = array('entityTypeId' => $currentObject->entityTypeId, 'history' => $currentObject->history, 'datesOfExistence' => $currentObject->datesOfExistence, 'places' => $currentObject->places, 'legalStatus' => $currentObject->legalStatus, 'functions' => $currentObject->functions, 'mandates' => $currentObject->mandates, 'internalStructures' => $currentObject->internalStructures, 'generalContext' => $currentObject->generalContext, 'rules' => $currentObject->rules, 'sources' => $currentObject->sources, 'revisionHistory' => $currentObject->revisionHistory, 'descriptionIdentifier' => $currentObject->descriptionIdentifier, 'corporateBodyIdentifiers' => $currentObject->corporateBodyIdentifiers);
				}
			}
          ...
          case 'repositoryDetail':
			...
			if (($childNode = $importDOM->xpath->query('.', $item)) !== null)
			{
				foreach ($nodeList2 as $item)
			    {
			      if (($childNode = $importDOM->xpath->query('./authorizedFormOfName', $item)) !== null)
			      {
				  	  $currentObject->authorizedFormOfName = $childNode->item(0)->nodeValue;
			      }
			      if (($childNode = $importDOM->xpath->query('./identifier', $item)) !== null)
			      {
				  ...
				  $this->repositoryValues = array('authorizedFormOfName' => $currentObject->authorizedFormOfName, 'identifier' => $currentObject->identifier, 'openingTimes' => $currentObject->openingTimes, 'geoculturalContext' => $currentObject->geoculturalContext, 'holdings' => $currentObject->holdings, 'findingAids' => $currentObject->findingAids, 'internalStructures' => $currentObject->internalStructures, 'internalStructures' => $currentObject->internalStructures, 'uploadLimit' => $currentObject->uploadLimit, 'collectingPolicies' => $currentObject->collectingPolicies, 'buildings' => $currentObject->buildings, 'accessConditions' => $currentObject->accessConditions, 'disabledAccess' => $currentObject->disabledAccess, 'researchServices' => $currentObject->researchServices, 'reproductionServices' => $currentObject->reproductionServices, 'publicFacilities' => $currentObject->publicFacilities, 'descInstitutionIdentifier' => $currentObject->descInstitutionIdentifier, 'descRules' => $currentObject->descRules, 'descSources' => $currentObject->descSources, 'descRevisionHistory' => $currentObject->descRevisionHistory);
				}
			}
			
            break;
      
          case 'donorAddress' || 'authorityAddress' || 'repositoryAddress':
		      if (($childNode = $importDOM->xpath->query('.', $item)) !== null)
		      {
		        foreach ($nodeList2 as $item)
		        {
		          if (($childNode = $importDOM->xpath->query('./addressline15', $item)) !== null)
		          {
			      	  $primaryContact = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline12', $item)) !== null)
		          {
				      $contactPerson = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline22', $item)) !== null)
		          {
				      $email = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline11', $item)) !== null)
		          {
				      $fax = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline7', $item)) !== null)
		          {
				      $telephone = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline10', $item)) !== null)
		          {
				      $cell = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline9', $item)) !== null)
		          {
				      $website = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline1', $item)) !== null)
		          {
				      $streetAddress = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline2', $item)) !== null)
		          {
				      $city = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline3', $item)) !== null)
		          {
				      $region = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline5', $item)) !== null)
		          {
				      $countryCode = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline6', $item)) !== null)
		          {
				      $postalCode = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline17', $item)) !== null)
		          {
				      $postalAddress = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline18', $item)) !== null)
		          {
				      $postalCity = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline19', $item)) !== null)
		          {
				      $postalRegion = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline20', $item)) !== null)
		          {
				      $postalCountryCode = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline21', $item)) !== null)
		          {
				      $postalPostCode = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline13', $item)) !== null)
		          {
				      $latitude = $childNode->item(0)->nodeValue;
		          }
		          if (($childNode = $importDOM->xpath->query('./addressline14', $item)) !== null)
		          {
				      $longitude = $childNode->item(0)->nodeValue;
		          }
			      	$this->addressValues = array('primaryContact' => $primaryContact, 'contactPerson' => $contactPerson, 'email' => $email, 'fax' => $fax, 'telephone' => $telephone, 'cell' => $cell, 'website' => $website, 'streetAddress' => $streetAddress, 'city' => $city, 'region' => $region, 'countryCode' => $countryCode, 'postalCode' => $postalCode, 'postalAddress' => $postalAddress, 'postalCity' => $postalCity, 'postalRegion' => $postalRegion, 'postalCountryCode' => $postalCountryCode, 'postalPostCode' => $postalPostCode, 'latitude' => $latitude, 'longitude' => $longitude);
				}
			}
...
*/
  ...
  {
  echo "addressValues['contactPerson']".$addressValues['contactPerson']."<br>";
	  if ($addressValues['contactPerson'] !== null)
	  {
