        ...
        <?php foreach ($acl as $objectId => $actions): ?>
          <tr>
            <td colspan="<?php echo $tableCols ?>"><strong>
              <?php if (1 < $objectId): ?>
                <?php echo esc_entities(render_title(QubitServiceProvider::getById($objectId))) ?>
              <?php else: ?>
                <em><?php echo __('All %1%', array('%1%' => lcfirst(sfConfig::get('app_ui_label_service_provider')))) ?></em>
              <?php endif; ?>
            </strong></td>
          </tr>
