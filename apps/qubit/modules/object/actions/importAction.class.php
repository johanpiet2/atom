...
class ObjectImportAction extends sfAction
{
  public function execute($request)
  {
    ...
    QubitXMLImport::addLog("Import not of supported tmp_name : ". $file['tmp_name'], "", get_class($this), false);
    QubitXMLImport::addLog("Import not of supported : ". $file['name'], "", get_class($this), false);
      ...
      foreach ($files as $importFile)
      {
        ...
        switch (strtolower(pathinfo($importFile, PATHINFO_EXTENSION)))
        {
          ...
          case 'xml':
            ...
            if (isset($this->getRoute()->resource)) $importer->setParent($this->getRoute()->resource);
       		$importer->import($importFile, array('strictXmlParsing' => false), strtolower(pathinfo($importFile, PATHINFO_EXTENSION)));
      ...
      try
      {
        ...
        switch ($importType)
        {
          case 'csv':
            ...
            if (isset($this->getRoute()->resource)) $importer->setParent($this->getRoute()->resource);
            $importer->import($file['tmp_name'], $request->csvObjectType);QubitXMLImport::addLog("Import xmlFile: 1>" . $file['tmp_name'] ."<2", "", get_class($this), true);
          ...
          case 'xml':
            ...
            $importer->import($file['tmp_name'],$request->csvObjectType, array('strictXmlParsing' => false), strtolower(pathinfo($file['tmp_name'], PATHINFO_EXTENSION)));
