...
class ReportsReportUpdatesAction extends sfAction
{
    ...
    protected function addField($name)
    {
        switch ($name) {
        case 'className':
            $choices = array('QubitInformationObject' => "Archival Description", 'QubitActor' => "Authority Record", 'QubitRepository' => "Repository", 'QubitRegistry' => "Register", 'QubitPresevationObject' => 'Preservation', 'QubitAccessObject' => "Access", 'QubitPhysicalObject' => "Physical Storage", 'QubitBookoutObject' => "Booked Out", 'QubitBookinObject' => "Booked In"); //, 'QubitTerm' => sfConfig::get('app_ui_label_term') removed
        ...
        case 'repositories':
            ...
            $criteria = QubitCultureFallback::addFallbackCriteria($criteria, 'QubitRepository');
...
/*         case 'formats':
            $this->form->setValidator('formats', new sfValidatorString);
            $choices = array();
            $choices[null] = null;
            ...
            $this->form->setValidator('formats', new sfValidatorString);
            $this->form->setWidget('formats', new sfWidgetFormSelect(array('choices' => $choices)));
            break;
*/
		case 'usability':
...
/*		case 'hard':
			$this->form->setValidator('hard', new sfValidatorString);
			$choices = array();
            $choices[null] = null;
            foreach (QubitTerm::getByParentId(QubitTerm::HARDS_ID) as $item) {
                $choices[$item->id] = $item;
            }
            ////array_splice($choices, 1, 1);
            $choices[null] = null;
            $this->form->setWidget('hard', new sfWidgetFormSelect(array('choices' => $choices)));
			break;

		case 'digital':
			$this->form->setValidator('digital', new sfValidatorString);
			$choices = array();
            $choices[null] = null;
            foreach (QubitTerm::getByParentId(QubitTerm::DIGITALS_ID) as $item) {
                $choices[$item->id] = $item;
            }
            //array_splice($choices, 1, 1);
            $choices[null] = null;
            $this->form->setWidget('digital', new sfWidgetFormSelect(array('choices' => $choices)));
			break;
*/
		case 'refusal':
    ...
    public function execute($request)
    {
        ...
        $defaults = array('className' => 'QubitBookoutObject', 'dateStart' => date('Y-m-d', strtotime('-1 month')), 'dateEnd' => date('Y-m-d'), 'dateOf' => 'CREATED_AT', 'publicationStatus' => 'all', 'limit' => '10', 'sort' => 'updatedDown');
    ...
    public function doSearch()
    {
        ...
        case 'QubitPresevationObject':
            $nameColumn = 'authorized_form_of_name';
            $this->nameColumnDisplay = 'Name';
            $criteria = QubitPresevationObject::addGetOnlyPreservationObjectCriteria($criteria);
            break;

        case 'QubitAccessObject':
            $nameColumn = 'authorized_form_of_name';
            $this->nameColumnDisplay = 'Name';
        ...
        case 'QubitPhysicalObject':
            $nameColumn = 'authorized_form_of_name';
            $this->nameColumnDisplay = 'Name';
            $criteria = QubitPhysicalObject::addGetOnlyPhysicalObjectCriteria($criteria);
            break;

        case 'QubitBookoutObject':
            $nameColumn = 'authorized_form_of_name';
            $this->nameColumnDisplay = 'Name';
            $criteria = QubitBookoutObject::addGetOnlyBookoutObjectCriteria($criteria);
            break;

        case 'QubitBookinObject':
            $nameColumn = 'name';
            $this->nameColumnDisplay = 'ID';
            $criteria = QubitBookinObject::addGetOnlyBookinObjectCriteria($criteria);
           break;
...
//add actor to display
//		 	$criteria->addJoin(QubitInformationObject::ID, QubitEvent::OBJECT_ID);
//		 	$criteria->addJoin(QubitActor::ID, QubitEvent::OBJECT_ID);
//            $criteria->add(QubitEvent::OBJECT_ID, QubitInformationObject::ID, Criteria::EQUAL);
			...
			if (null !== $this->form->getValue('hard') && $this->form->getValue('hard') != '') 
            {
				$criteria->addJoin(QubitRelation::OBJECT_ID, QubitInformationObject::ID);
				$criteria->addJoin(QubitRelation::OBJECT_ID, QubitPresevationObject::OBJECT_ID);
				$criteria->addJoin(QubitPresevationObject::ID, QubitPresevationObjectI18n::ID);
				$criteria->add(QubitPresevationObject::HARD_ID, $this->form->getValue('hard'), Criteria::EQUAL);
            }

			if (null !== $this->form->getValue('digital') && $this->form->getValue('digital') != '') 
            {
				$criteria->addJoin(QubitRelation::OBJECT_ID, QubitInformationObject::ID);
				$criteria->addJoin(QubitRelation::OBJECT_ID, QubitPresevationObject::OBJECT_ID);
				$criteria->addJoin(QubitPresevationObject::ID, QubitPresevationObjectI18n::ID);
				$criteria->add(QubitPresevationObject::DIGITAL_ID, $this->form->getValue('digital'), Criteria::EQUAL);
            }
        ...
        // Page results
        QubitXMLImport::addLog("this->pager...".$criteria->toString(), "getRelationsBySubjectId", get_class($this), true); 
