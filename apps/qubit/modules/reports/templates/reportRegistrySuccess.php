...
<?php slot('title') ?>
  <h1 class="multiline">
    ...
    <?php echo __('Browse Registries Report') ?>
  </h1>
<?php end_slot() ?>
...
<section class="sidebar-widget">
	...
	<body onload="javascript:NewCal('dateStart','ddmmyyyy',false,false,24,true);renderCalendar('dateStart','div0');
		...
		<div>
			...
			<?php echo $form->renderFormTag(url_for(array('module' => 'reports', 'action' => 'reportRegistry')), array('method' => 'get')) ?>
  ...
  <table class="table table-bordered" border="1" cellpadding="0" cellspacing="0" bordercolor="#999999">
    ...
    <?php foreach ($pager->getResults() as $result): ?>
        <tr class="<?php echo 0 == @++$row % 2 ? 'even' : 'odd' ?>">
			<?php if (isset($result->corporateBodyIdentifiers)) { ?> <td><?php echo link_to($result->corporateBodyIdentifiers, array('module' => 'registry', 'action' => 'index', 'source' => $result->id)) ?></td> <?php } else { ?> <td>-</td> <?php }	?>
